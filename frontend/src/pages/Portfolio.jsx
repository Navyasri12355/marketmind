...
const STORAGE_KEY = 'marketmind_holdings'
const COLORS = ['#f59e0b', '#14b8a6', '#a78bfa', '#60a5fa', '#22c55e', '#f97316', '#e879f9', '#38bdf8']

function StatCard({ label, value, sub, color, delay }) {
  return (
    <motion.div className="card" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      transition={{ delay }} style={{ position: 'relative', overflow: 'hidden' }}>
...
function DeleteButton({ onClick }) {
  ...
  return (
    <button
      onClick={handleClick}
      title={confirm ? 'Click again to confirm removal' : 'Remove holding'}
      style={{
        background: confirm ? 'rgba(239,68,68,0.12)' : 'none',
        border: confirm ? '1px solid rgba(239,68,68,0.35)' : '1px solid transparent',
        borderRadius: 6,
...
export default function Portfolio() {
  ...
  const addHolding = () => {
    ...
    const qty       = parseInt(newHolding.qty)
    const avg_price = parseFloat(newHolding.avg_price)
    if (isNaN(qty) || qty <= 0)       { setError('Quantity must be a positive number.'); return }
    if (isNaN(avg_price) || avg_price <= 0) { setError('Average price must be a positive number.'); return }

    setError('')
    const ticker      = newHolding.ticker.toUpperCase().trim()
    const finalTicker = ticker.includes('.') ? ticker : `${ticker}.NS`

    // Prevent duplicate tickers
    if (holdings.some(h => h.ticker === finalTicker)) {
      setError(`${finalTicker} is already in your portfolio. Remove it first to update.`)
      return
    }

    setHoldings(prev => [...prev, {
      ticker:    finalTicker,
      name:      newHolding.name.trim() || ticker,
      qty,
      avg_price,
    }])
    ...
    <div className="page">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <div className="page-subtitle">Live P&L · Allocation · Holdings saved automatically</div>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-primary" onClick={runXRay} disabled={loading || holdings.length === 0}>
            {loading
              ? <><div className="spinner" style={{ width: 14, height: 14 }} /> Fetching prices</>
              : '◎ Run X-Ray'}
          </button>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 16, padding: '14px 16px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr 80px 110px auto', gap: 8, alignItems: 'flex-end' }}>
          ...
          <button className="btn btn-teal" style={{ fontSize: 12, padding: '7px 14px' }} onClick={addHolding}>
            + Add
          </button>
        </div>
      ...
      {holdings.length > 0 && (
        <div className="card" style={{ marginBottom: 20, padding: 0, overflow: 'hidden' }}>
          <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Saved locally ·  Auto-restored on refresh</span>
          </div>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Stock', 'Qty', 'Avg Price', 'Invested',
                  result ? 'Current' : null,
                  result ? 'P&L' : null,
              ...
              {holdings.map((h, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td style={{ padding: '11px 16px' }}>
                      <DeleteButton onClick={() => removeHolding(i)} />
                    </td>
                  </motion.tr>
                )
              })}
            ...
            <div className="card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {[...result.holdings].sort((a, b) => b.pnl_pct - a.pnl_pct).map((h, i) => {
                  ...
                  const width = max > 0 ? Math.abs(h.pnl_pct) / max * 100 : 0
                  return (
                    <div key={i}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                        <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{h.name?.split(' ')[0]}</span>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 500, color: h.pnl_pct >= 0 ? 'var(--green)' : 'var(--red)' }}>
      ...
      {!result && !loading && holdings.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 40px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)' }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
            Enter a ticker (e.g. INFY), quantity, and average buy price. Holdings are saved to your browser and restored automatically.
          </div>
        </div>
      )}
    </div>
